...
type ElasticIPRenderer interface {
	RenderElasticIP(actual, expected, changes *ElasticIP) error
}
...
func (e *ElasticIP) Run(c *fi.RunContext) error {
	...
	target := c.Target.(ElasticIPRenderer)
	return target.RenderElasticIP(a, e, changes)
}
...
func (t *AWSAPITarget) RenderElasticIP(a, e, changes *ElasticIP) error {
	var publicIP *string
...
func (t *BashTarget) RenderElasticIP(a, e, changes *ElasticIP) error {
	t.CreateVar(e)
	if a == nil {
