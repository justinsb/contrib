...
type DHCPOptionsRenderer interface {
	RenderDHCPOptions(actual, expected, changes *DHCPOptions) error
}
...
func (e *DHCPOptions) Run(c *fi.RunContext) error {
	...
	target := c.Target.(DHCPOptionsRenderer)
	return target.RenderDHCPOptions(a, e, changes)
}
...
func (t *AWSAPITarget) RenderDHCPOptions(a, e, changes *DHCPOptions) error {
	if a == nil {
...
func (t *BashTarget) RenderDHCPOptions(a, e, changes *DHCPOptions) error {
	t.CreateVar(e)
	if a == nil {
		glog.V(2).Infof("Creating DHCPOptions with Name:%q", *e.Name)
